...
```
  ...
  The following is an R script which takes the summarized values from the spatial join and plots them with colors and legends.
    ...
    plotmonth <- function(month,nclr=5) {
      screen(month)
      #compute class intervals on all data!
      ...
      pltclass <- classIntervals(monthdata$abund*100, nclr, style="fixed", fixedBreaks=mapclass['brks']$brks)
      pcol <- findColours(pltclass, colpal, cutlabels=T)

      # Write SVG files out for each plot
      svg(sprintf('map%03d.svg',month), width=15, height=9)
      plot(monthdata, xlim=c(-119.0,-85.75), ylim=c(14.0, 32.74), col=pcol, border=FALSE)
      plot(mx , col="black", add=T)
      plot(subset(p, MES_COL == month), add=T, pch=20, col="orange", cex=0.8)

      #legend(-97,30, title="Buntings as a percent of total collections", legend=names(attr(pcol,"table")), fill=attr(pcol,"palette"), cex = 0.75, bty = "n" )
      #title(months[month])
      dev.off()
    }
